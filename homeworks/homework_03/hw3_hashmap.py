...
class HashMap:
    ...
    def put(self, key, value):
        ...
        for i, entry in enumerate(self.hash_map[ind]):
            if entry == put_entry:
                self.hash_map[ind].pop(i)
                break
        self.hash_map[ind].append(put_entry)

        control_size = self.bucket_num * 0.7
        if len([lst for lst in self.hash_map if lst]) > control_size:
            self._resize()
        pass
