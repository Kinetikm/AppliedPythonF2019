...
class CSRMatrix:
    ...
    def __init__(self, matrix):
        ...
        if isinstance(matrix, tuple) and len(matrix) == 3:
            n = 0
            for i in range(max(matrix[0])+1):
                n += list(matrix[0]).count(i)
                self.IA.append(n)
            self.A = list(matrix[2])
            self.JA = list(matrix[1])
            self.max_column = max(matrix[1]) + 1
