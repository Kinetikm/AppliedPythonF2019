...
# Делать .terminate() можно,в данном случае,очередь повредится и при попытке достать что-либо выбросит исключения
# (у меня были EOFError и Broken Pipe Error).
# Поэтому в процессах у меня стоит обработчик с Exception(читайте п.2)
#
# 2. Если очередь имеет переменный размер(может как уменьшаться так и увеличиваться), то тогда нельзя использовать
# queue.size() или queue.empty(), согласно документации эти методы ненадежны в многопроцессорном режиме.
# По этой причине я использую q_get_timeout, по истечению которого выбрасывается исключение (queue.get(timeout=)),
# что является индикатором того,что в очереди больше ничего нет и тогда я записываю в "shared list" pid этого процесса.
# Если так получилось, что все воркеры сообщили о том, что очередь пуста,
# тогда мэнеджер убивает все процессы и завершает работу.
